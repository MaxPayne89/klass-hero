defmodule KlassHeroWeb.Provider.DashboardTeamTest do
  ...
  describe "self-staffing (#969)" do
    import KlassHero.EmailTestHelper
    import Swoosh.TestAssertions

    defp log_in_named_provider(conn, name) do
      user = KlassHero.AccountsFixtures.user_fixture(%{intended_roles: [:provider], name: name})
    ...
    test "clicking the button opens the form pre-filled from the account", %{conn: conn} do
      ...
      assert has_element?(
               view,
               "input[name='staff_member_schema[email]'][value='#{user.email}']"
             )
    end

    test "submitting creates a linked accepted row, grants :staff, sends no email", %{
      conn: conn
    } do
      %{conn: conn, user: user, provider: provider} = log_in_named_provider(conn, "Maxi Founder")
