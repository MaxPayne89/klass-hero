...
**CQRS reads:** CQRS here means a read model separate from the write model — **not** a second table maintained asynchronously. The default read is an ordinary query over the write table, returning the entity struct, which under schema-as-struct *is* the DTO; queries go in the context module or a context-root submodule (`provider/programs.ex`, `provider/assignments.ex`). A **projection is the exception and must be justified** by naming the query a facade call (ADR 0015) or a same-context query cannot serve — five were retired for failing that test (ADR 0023). One remains: `provider/projections/provider_session_details.ex` with its read table `provider/session_detail.ex`. Build one, once justified, on `KlassHero.Shared.Projection`. See `.claude/rules/domain-architecture.md` for the three read-side kinds and what a projection costs.
