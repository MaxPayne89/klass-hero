...
**Context boundaries:** Not tooling-enforced (`boundary` library removed). Cross-context isolation is a convention — call other contexts only through their root module's public API (e.g. `KlassHero.Family`), never reach into their internals. For cross-context *reads*, **call the owning context's root facade directly** (ADR 0015) — at every layer, adapters included. An ACL under `acl/` is for genuine translation only (error remapping, business-rule masking, cycle-breaking table access); a projection is for a read path a per-render facade call can't serve, and must say which (ADR 0023). Never call another context's Repo/schemas directly.

**CQRS reads:** CQRS here means a read model separate from the write model — **not** a second table maintained asynchronously. The default read is an ordinary query over the write table, returning the entity struct, which under schema-as-struct *is* the DTO; queries go in the context module or a context-root submodule (`provider/programs.ex`, `provider/assignments.ex`). A **projection is the exception and must be justified** by naming the query a facade call (ADR 0015) or a same-context query cannot serve — four were retired for failing that test (ADR 0023). Two remain: `provider/projections/provider_session_details.ex` with its read table `provider/session_detail.ex`, and `messaging/conversation_summaries.ex`. Build one, once justified, on `KlassHero.Shared.Projection`. See `.claude/rules/domain-architecture.md` for the three read-side kinds and what a projection costs.
