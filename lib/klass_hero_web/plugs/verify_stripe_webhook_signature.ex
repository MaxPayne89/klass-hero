defmodule KlassHeroWeb.Plugs.VerifyStripeWebhookSignature do
  ...
  Stripe protocol:
  1. The `Stripe-Signature` header is a comma-separated list: `t=<timestamp>,v1=<signature>` (it may
     carry several `v1` entries during key rotation).
  2. Build the signed payload `"<timestamp>.<raw_body>"`.
  3. HMAC-SHA256 it with the endpoint signing secret (`whsec_…`), used as-is — Stripe does NOT
     base64-decode the secret (that is the Svix/Resend scheme).
