defmodule KlassHeroWeb.StripeWebhookController do
  ...
  Terminal session events (`verified` / `requires_input` / `canceled`) are normalised and handed to
  `RecordIdentityVerificationOutcome`; `processing` and any other event are acknowledged with 200
  but not acted on. Always replies 200 — a non-2xx would make Stripe retry indefinitely.
  """
