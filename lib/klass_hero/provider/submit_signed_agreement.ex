defmodule KlassHero.Provider.SubmitSignedAgreement do
  ...
  def execute(%{kind: kind, provider_id: provider_id, signed_by_name: signed_by_name}) do
    ...
    # Resolve AND advance the step (which enforces its `requires` prerequisites, e.g. the business
    # steps needing an approved responsible-person identity) before persisting any consent evidence,
    # so a provider whose track lacks the step OR whose prerequisites are unmet never leaves an orphan
    # SignedAgreement behind. `agreement.id` is generated by `new/1` and becomes the inserted row's id,
    # so it is the same evidence_ref whether read before or after the insert.
    with {:ok, profile} <- Provider.get_provider_profile(provider_id),
         ...
         {:ok, updated} <- VettingCase.auto_approve_step(case_, key, agreement.id),
