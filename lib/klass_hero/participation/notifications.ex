defmodule KlassHero.Participation.Notifications do
  ...
  it needs to refetch, never a `%IntegrationEvent{}`. Nothing in the web layer knows
  ...
  defp message(%IntegrationEvent{event_type: type, payload: %{session_id: session_id}}) when type in @session_events,
    do: {:session_changed, session_id}

  defp message(%IntegrationEvent{event_type: :sessions_generated, payload: %{program_id: program_id}}),
    do: {:sessions_generated, program_id}

  defp message(%IntegrationEvent{event_type: type, payload: payload}) when is_map_key(@attendance_kinds, type) do
    {:attendance_changed,
     %{
  ...
  defp message(%IntegrationEvent{event_type: type}) when type in @note_events, do: :session_notes_changed

  defp message(%IntegrationEvent{}), do: nil

  defp topics(%IntegrationEvent{payload: payload} = event) do
    provider_topics(event) ++ child_topics(payload)
  end

  # Session notes know their provider; everything else has to ask the catalog.
  defp provider_topics(%IntegrationEvent{event_type: type, payload: payload}) when type in @note_events do
  ...
  defp provider_topics(%IntegrationEvent{payload: payload} = event) do
