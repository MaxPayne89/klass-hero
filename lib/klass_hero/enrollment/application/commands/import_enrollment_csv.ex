defmodule KlassHero.Enrollment.Application.Commands.ImportEnrollmentCsv do
  ...
  # header-only CSVs pass validate_headers/1 but yield zero {:ok,_} rows;
  # without this guard the use case would silently return {:ok, %{created: 0}}.
  ...
  # Separates inner (:continue/:halted) from outer (:cont/:halt) reduce tags to prevent
  # accidental coupling if the inner accumulator shape changes.
  ...
  # file_row is 2-based (header = row 1, first data row = 2) via Stream.with_index(2).
  ...
  # Lazy-load DB keys per program — avoids preloading all provider programs when the CSV touches only a few.
