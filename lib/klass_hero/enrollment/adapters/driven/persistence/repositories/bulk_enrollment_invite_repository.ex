defmodule KlassHero.Enrollment.Adapters.Driven.Persistence.Repositories.BulkEnrollmentInviteRepository do
  ...
  def create_one(attrs) when is_map(attrs) do
    db_interaction operation: :create_one, entity: "bulk_enrollment_invite" do
  ...
  def list_existing_keys_for_programs(program_ids) when is_list(program_ids) do
    db_interaction operation: :list_existing_keys_for_programs, entity: "bulk_enrollment_invite" do
      BulkEnrollmentInviteSchema
  ...
  def invite_exists?(program_id, guardian_email, child_first_name, child_last_name)
    ...
    db_interaction operation: :invite_exists, entity: "bulk_enrollment_invite" do
  ...
  def get_by_id(id) when is_binary(id) do
    db_interaction operation: :get_by_id, entity: "bulk_enrollment_invite" do
  ...
  def get_by_token(token) when is_binary(token) do
    db_interaction operation: :get_by_token, entity: "bulk_enrollment_invite" do
      BulkEnrollmentInviteSchema
  ...
  def list_pending_without_token(program_ids) when is_list(program_ids) do
    db_interaction operation: :list_pending_without_token, entity: "bulk_enrollment_invite" do
      BulkEnrollmentInviteSchema
      |> where([i], i.program_id in ^program_ids)
  ...
  def list_by_program(program_id) when is_binary(program_id) do
    db_interaction operation: :list_by_program, entity: "bulk_enrollment_invite" do
      BulkEnrollmentInviteSchema
      |> where([i], i.program_id == ^program_id)
  ...
  def count_by_program(program_id) when is_binary(program_id) do
    db_interaction operation: :count_by_program, entity: "bulk_enrollment_invite" do
      BulkEnrollmentInviteSchema
      |> where([i], i.program_id == ^program_id)
  ...
  def delete(id) when is_binary(id) do
    db_interaction operation: :delete, entity: "bulk_enrollment_invite" do
      case Repo.get(BulkEnrollmentInviteSchema, id) do
        nil ->
          {:error, :not_found}
  ...
  def bulk_assign_tokens(id_token_pairs) when is_list(id_token_pairs) do
    db_interaction operation: :bulk_assign_tokens, entity: "bulk_enrollment_invite" do
  ...
  def transition_status(%{id: id}, attrs) when is_map(attrs) do
    db_interaction operation: :transition_status, entity: "bulk_enrollment_invite" do
  ...
  def reset_for_resend(%{id: id, status: status}) when status in @resendable_statuses do
    db_interaction operation: :reset_for_resend, entity: "bulk_enrollment_invite" do
      case Repo.get(BulkEnrollmentInviteSchema, id) do
        nil ->
          {:error, :not_found}
