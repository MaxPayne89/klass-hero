...
**~~Sessions are hand-created, not generated from the Program schedule.~~ Resolved (#1187).**
A Program's recurring schedule (meeting days + meeting start/end times + a date range) is now expanded into concrete **Sessions**, which carry `origin: :generated` and are kept in agreement with the schedule on every Program write. A Session a provider adds by hand is `origin: :manual` and is never swept. What a generated Session inherits from its Program — times, location, **Session Capacity** — is copied at generation and realigned on later edits.
